...
argument to pass to the internal RUN-PROGRAM"
    ...
    (etypecase specifier
      ...
      ((eql :interactive)
       #+allegro nil
       #+clisp :terminal
       #+(or clozure cmu ecl clasp mkcl sbcl scl) t)
      #+(or allegro clozure cmu ecl clasp lispworks mkcl sbcl scl)
      ((eql :output)
...
It returns a process-info plist with possible keys:
    ...
    (let* ((%command (%normalize-command command))
           ...
           (process*
             ...
             (with-current-directory (#-(or sbcl mkcl) directory)
               ...
               #+(or clozure cmu ecl clasp mkcl sbcl scl)
               (#-(or ecl clasp mkcl) progn #+(or ecl clasp mkcl) multiple-value-list
                (apply
      ...
      (flet ((prop (key value) (push key process-info-r) (push value process-info-r)))
        ...
        #+(or ecl clasp mkcl)
        (destructuring-bind #+(or ecl clasp) (stream code process) #+mkcl (stream process code) process*
  ...
  (defun %process-info-pid (process-info)
    (let ((process (getf process-info :process)))
      ...
      #+(or ecl clasp) (si:external-process-pid process)
    ...
    (or (getf process-info :exit-code)
        (let ((process (getf process-info :process)))
          (when process
            ...
            #+(and (or ecl clasp) os-unix) (ext:external-process-wait process)
            #+sbcl (sb-ext:process-wait process)
            ;; 2- extract result
            #+allegro (sys:reap-os-subprocess :pid process :wait t)
            #+clozure (nth-value 1 (ccl:external-process-status process))
            #+(or cmu scl) (ext:process-exit-code process)
            #+(or ecl clasp) (nth-value 1 (ext:external-process-status process))
            #+lispworks
  ...
  (defun %system (command &rest keys
    ...
    #+(or abcl cormanlisp clisp ecl clasp gcl genera (and lispworks os-windows) mkcl xcl)
    (let ((%command (%redirected-system-command command input output error-output directory)))
      ...
      (with-current-directory ((unless (os-unix-p) directory))
        ...
        #+(or ecl clasp) (let ((*standard-input* *stdin*)
...
or an indication of failure via the EXIT-CODE of the process"
    ...
    #-(or abcl allegro clisp clozure cmu cormanlisp ecl clasp gcl lispworks mcl mkcl sbcl scl xcl)
    (error "RUN-PROGRAM not implemented for this Lisp")
    (flet ((default (x xp output) (cond (xp x) ((eq output :interactive) :interactive))))
      (apply (if (or force-shell
                     #+(or clisp ecl clasp) (or (not ignore-error-status) t)
