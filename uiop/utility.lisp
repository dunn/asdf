         ...
         `(defmacro ,def* (name formals &rest rest)
            (destructuring-bind (name &key (supersede t))
              ...
              `(progn
                 ...
                 ,@(when (or supersede #+(or ecl clasp) t)
                     `((undefine-function ',name)))
                 ,@(when (and #+(or ecl clasp) (symbolp name)) ; fails for setf functions on ecl
...
(with-upgradability ()
  (defparameter +simple-condition-format-control-slot+
    ...
    #+(or ecl clasp mkcl) 'si::format-control
    #+(or gcl lispworks) 'conditions::format-string
    #+sbcl 'sb-kernel:format-control
    #-(or abcl allegro clisp clozure cmu ecl clasp gcl lispworks mkcl sbcl scl) nil
