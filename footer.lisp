...
#+(or abcl clisp clozure cmu ecl clasp mkcl sbcl)
(with-upgradability ()
  (if-let (x (and #+clisp (find-symbol* '#:*module-provider-functions* :custom nil)))
    (eval `(pushnew 'module-provide-asdf
                    ...
                    #+(or cmu ecl clasp) ext:*module-provider-functions*
  ...
  (progn
    ...
    #+(or (and ecl win32) (and mkcl windows) (and clasp windows))
    (unless (assoc "asd" #+(or ecl clasp) ext:*load-hooks* #+mkcl si::*load-hooks* :test 'equal)
      (appendf #+(or ecl clasp) ext:*load-hooks* #+mkcl si::*load-hooks* '(("asd" . si::load-source))))

    (setf #+(or ecl clasp) ext:*module-provider-functions* #+mkcl mk-ext::*module-provider-functions*
          (loop :for f :in #+(or ecl clasp) ext:*module-provider-functions*
                #+mkcl mk-ext::*module-provider-functions*
                :collect
