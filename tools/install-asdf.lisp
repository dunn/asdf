...
(defun module-directory ()
  ...
  #+scl #p"file://modules/"
  #+xcl ext:*xcl-home*
  #-(or allegro clisp clozure cmu ecl clasp gcl lispworks mkcl sbcl scl xcl)
...
(defun module-fasl (name)
  ...
  (flet ((pathname-key (x)
    ...
    (first (sort (directory (merge-pathnames* (strcat name ".*") (module-directory)))
                 #'> :key #'pathname-key)))
  #+(or clisp clozure cmu ecl clasp gcl lispworks mkcl sbcl scl xcl)
...
(defun object-file (name &optional (type :object))
  #-(or ecl clasp mkcl) (progn name type (assert nil))
  #+(or ecl clasp) (compile-file-pathname name :type type)
...
(defun install-asdf-as-module ()
  (nest
   (let* ((asdf.lisp (system-relative-pathname :uiop "../build/asdf.lisp"))
          ...
          #+(or ecl clasp mkcl) (asdf.o (object-file asdf.fasl :object))
          #+(or ecl clasp mkcl) (asdf.a (object-file asdf.fasl :lib))))
   (with-file-replacement (asdf.fasl))
   #+(or ecl clasp mkcl) (with-file-replacement (asdf.o))
   #+(or ecl clasp mkcl) (with-file-replacement (asdf.a))
   (progn
     (compile-file* asdf.lisp :output-file asdf.fasl
                    #+(or ecl clasp mkcl) :object-file #+(or ecl clasp mkcl) asdf.o)
     #+(or ecl mkcl)
...
#+(or allegro clisp clozure cmu ecl clasp gcl lispworks scl xcl)
